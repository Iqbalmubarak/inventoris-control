...
class UserController extends Controller
{
  public function index(Request $request)
  {
    ...
    if(Sentinel::getUser()->hasAccess(['users.part-data'])){
      $datas = $datas->join('role_users','users.id','=','role_users.user_id')->where('role_users.role_id','<>','1');
    }

    if(Sentinel::getUser()->id!=1){
      $datas = $datas->where('users.id','!=','1');
    }

    if($request->user=='actived'){
      $datas = $datas->whereraw('users.id in (select user_id from activations)');
    }
    else if($request->user=='deactived'){
      $datas = $datas->whereraw('users.id not in (select user_id from activations)');
    }
    // dd($datas->lastItem());
  ...
  public function create()
  {
    ...
    return view('backend.users.create',compact('satker','role'));
  }
  ...
  public function store(Request $request)
  {
    ...
    try {
      $data = $request->all();
      $data['satker_id'] = $request->satuan_kerja;
      ...
      if($user->id){
        ...
        if(config('value.setting_send_email.status')){
          ...
          $data = (object) $data;
          Mail::send('auth.email',array('data' => $data),function($message) use ($to_email)
          {
            $message->to($to_email)->subject('Verifikasi SigUp in APD PADAG');
  ...
  public function edit($id)
  {
      ...
      return view('backend.users.edit',compact('data','role','satker'));
  }
