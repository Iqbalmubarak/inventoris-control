...
class inventorisRequestController extends Controller
{
      ...
      public function acc_one(Request $request, $id)
      {
        try {
          ...
          ->join('inventoris_detail','inventoris.id','=','inventoris_detail.inventoris_id')
          ->select('inventoris_detail.total')
          ->where('inventoris_detail.sumber_id',1)
          ->where('barang_id',$inreqde->barang_id)
          ->where('inventoris_detail.sumber_id',$request->sumber)
          ->where('satuan_id',$inreqde->satuan_id)->get();
      ...
      public function acc(Request $request, $id)
      {
          ...
          foreach($details as $detail)
          {
            ...
            for ($y=0; $y<$i ; $y++) {
              ...
              ->where('satuan_id',$satuan_id[$y])->get();
              ...
              if($total[$y]<=$sum){
                $tot[$y]=1;
              try{
                $inventory = DB::table('inventoris')
                ->where('satker_id',1)
                ->where('barang_id',$request->barang_id)
                ->where('satuan_id',$request->satuan_id)->first();
                $detail = DB::table('inventoris_detail')
                ->where('inventoris_id',$inventory->id)->get();

                inventoris::create([
                  'satker_id' => $satker_id,
                  'barang_id' => $barang_id[$y],
                  'satuan_id'=>$satuan_id[$y],]);

                $old = inventoris::where('satker_id',$satker_id)
                ->where('barang_id',$barang_id[$y])
                ...
                try{
                  inde::create([
                    ...
                    $add = DB::table('inventoris')
                    ->join('inventoris_detail','inventoris.id','=','inventoris_detail.inventoris_id')
                    ->select('inventoris_request.id')
                    ->where(['satker_id',$user],['barang_id',$barang_id[$y]],['satuan_id',$satuan_id[$y]] )->first();
                }
                catch(\Exception $e){
                  $tanggal = date('Y-m-d');
                  $sample = inde::where('inventoris_id',$old->id)
      ...
      public function reject(Request $request, $id)
      {
          try {
            ...
            foreach ($inreqde as $data) {
                ...
                ->where('inventoris.atuan_id',$data->satuan_id)->where('inventoris.barang_id',$data->barang_id)
