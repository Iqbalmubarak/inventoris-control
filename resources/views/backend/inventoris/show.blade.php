  ...
  <div class="kt-portlet__body">
    ...
    <thead>
        ...
        <tr>
          ...
          <th>: {{$data->details->sum('total')-$data->keluar()->sum('total')}}</th>
          <th></th>
          <th></th>
        </tr>
      </thead>
      ...
      <tbody>
        @foreach($data->details as $detail)
          <tr>
            ...
            <td>
              @if($detail->status==0 || Sentinel::getUser()->hasAccess('inventoris.all-data'))
                  {{$detail->sumber->name}}
              @else
                Dinas Kesehatan
              @endif
...
<div class="kt-portlet kt-portlet--mobile">
  ...
  <div class="kt-portlet__body">
    <table class="table table-striped- table-bordered table-hover table-checkable" id="tbl_inventorisDetailKeluar">
      <thead>
        <tr>
          <th>@lang('inventoris.field.keterangan')</th>
          <th>@lang('inventoris.field.banyak')</th>
          <th>@lang('inventoris.field.tanggal')</th>
          @if(Sentinel::getUser()->hasAccess(['inventorisDetail.updt']) || Sentinel::getUser()->hasAccess(['inventorisDetail.destroy']))
          <th>@lang('global.app_action')</th>
      ...
      <tbody>
        @foreach($data->keluar as $keluar)
          <tr>
            ...
            @if(Sentinel::getUser()->hasAccess(['inventorisKeluar.updt']) || Sentinel::getUser()->hasAccess(['inventorisKeluar.destroy']))
              <td>
                @if(Sentinel::getUser()->hasAccess(['inventorisKeluar.updt']))
                <a class="btn btn-success btn-sm" href="javascript:void(0)" onclick="editmodalkeluar({{$keluar}})">@lang('global.app_edit')</a>
                @endif
                @if(Sentinel::getUser()->hasAccess(['inventorisKeluar.destroy']))
                <a class="btn btn-danger btn-sm" href="javascript:void(0)" onclick="confirmdeletekeluar({{$keluar->id}})">@lang('global.app_delete')</a>
                @endif
              </td>
            @endif
          </tr>
...
<!--begin::Modal-->
<div class="modal fade" id="f_modal_create_keluar" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Create</h5>
      ...
      {{ Form::open(array('url' => route('inventorisKeluar.add',$data->id), 'files' => true, 'id'=>'form_inventorisKeluar_create')) }}
        <div class="modal-body">
          <div class="col-md-12">
            <div class="kt-form__group--inline">
              <div class="kt-form__label">
          ...
          <div class="col-md-12">
            ...
            <div class="col-md-12">
              <div class="kt-form__group--inline">
                ...
                <div class="kt-form__control">
                  <textarea  name="keterangan" class="form-control" id="k_keterangan_create">
                  </textarea>
                  @error('keterangan')
                    <div class="form-text text-danger">{{$message}}</div>
                  @enderror
...
<!--begin::Modal-->
<div class="modal fade" id="f_modal_edit_keluar" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
      ...
      {{ Form::open(array('method'=>'PATCH','url' => '#', 'files' => true, 'id'=>'form_inventorisKeluar_edit')) }}
        <div class="modal-body">
          <div class="col-md-12">
            <div class="kt-form__group--inline">
              <div class="kt-form__label">
              ...
              <div class="kt-form__control">
                <input type="date" name="tanggal" class="form-control" placeholder="Enter number" id="d_tanggal_update_keluar">
                @error('total')
                  <div class="form-text text-danger">{{$message}}</div>
                @enderror
              </div>
          ...
          <div class="col-md-12">
              <div class="kt-form__group--inline">
                ...
                <div class="kt-form__control">
                <input type="number" min="1" max="{{$data->details->sum('total')-$data->keluar()->sum('total')}}" name="total" class="form-control" placeholder="Enter number" id="n_total_update_keluar">
                  @error('total')
                    <div class="form-text text-danger">{{$message}}</div>
                  @enderror
                </div>
            ...
            <div class="col-md-12">
              <div class="kt-form__group--inline">
                ...
                <div class="kt-form__control">
                  <textarea  name="keterangan" class="form-control" id="k_keterangan_update_keluar">
                  </textarea>
                  @error('keterangan')
                    <div class="form-text text-danger">{{$message}}</div>
                  @enderror
  ...
  <script type="text/javascript">
    ...
    jQuery(document).ready(function() {
      ...
      var table = $('#tbl_inventorisDetailKeluar').DataTable({
        ...
        searchDelay: 500,
        processing: true,
        @if(Sentinel::getUser()->hasAccess(['inventorisKeluar.add']))
        dom: `<'row'<'col-sm-6 text-left'f><'col-sm-6 text-right'B>>
			<'row'<'col-sm-12'tr>>
			<'row'<'col-sm-12 col-md-5'i><'col-sm-12 col-md-7 dataTables_pager'lp>>`,
        buttons: [
            {
                text: "@lang('inventoris.add_keluar')",
                className: "btn btn-outline-primary",
                action: function ( e, dt, node, config ) {
                    openmodalcreatekeluar();
                }
            }
        ]
        @endif
    ...
    function editmodalkeluar(keluar) {
      emptyform();
      $("#d_tanggal_update1").val(keluar.tanggal);
      $("#n_total_update1").val(keluar.total);
      $("#k_keterangan_update").val(keluar.keterangan).trigger('change');
      $('#f_modal_edit_keluar').modal('show');
      var base = "{{url('/')}}";
      $('#form_inventorisKeluar_edit').attr('action', base+"/keluar-inventoris/{{$data->id}}/"+keluar.id);
    }
